...
export function CommunityFeed({ initialPosts }: { initialPosts?: Post[] }) {
  ...
  const loadPosts = useCallback(async (currentPage: number) => {
    ...
    try {
      ...
      }>(`/api/community?page=${currentPage}&limit=12`);
      setPosts(data.posts ?? []);
  ...
  async function createPost(e: React.FormEvent) {
    ...
    try {
      ...
      if (page === 1) {
        setPosts((prev) => [data.post, ...prev].slice(0, 12));
        setPagination((p) => ({
          ...p,
          total: p.total + 1,
          totalPages: Math.max(1, Math.ceil((p.total + 1) / p.limit)),
        }));
      } else {
        setPage(1);
      }
